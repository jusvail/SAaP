...
{
    public NFoldPatternIndicator(ComputingData computingData, int startIndex, int endIndex) : base(computingData,
        startIndex, endIndex)
    {
    }
    ...
    public override MatchResult Indicate()
    {
        ...
        try
        {
            for (var j = 0; j < range;)
            {
                ...
                while (j < range)
                {
                    ...
                    if (!firstLowerThan50d && Ori[j].Ending > D50[j])
                    {
                        j++;
                        continue;
                    }

                    firstLowerThan50d = true;
                    ...
                    // cci buy triggered 
                    var cciReadyToBuy = CciBs[j] == Models.CciIndicate.BuyTriggered;
                    ...
                    var sameWithY1 = Math.Abs(CalculationService.CalcTtm(Ori[j - 1].Ending, Ori[j].Ending));
                    var sameWithY2 = Math.Abs(CalculationService.CalcTtm(Ori[j - 2].Ending, Ori[j].Ending));
                    var sameWithY3 = Math.Abs(CalculationService.CalcTtm(Ori[j - 3].Ending, Ori[j].Ending));
                    var sameWithY4 = Math.Abs(CalculationService.CalcTtm(Ori[j - 4].Ending, Ori[j].Ending));
                    var sameWithY5 = Math.Abs(CalculationService.CalcTtm(Ori[j - 5].Ending, Ori[j].Ending));
                    ...
                    // if (pullback is < 20 and > -30)
                    // {
                    //     step2Progress += 20 + pullback;
                    // }


                    if (sameWithY1 < 2d) step2Progress += 5;
                    if (sameWithY2 < 2d) step2Progress += 5;
                    if (sameWithY3 < 2d) step2Progress += 5;
                    if (sameWithY4 < 2d) step2Progress += 5;
                    if (sameWithY5 < 2d) step2Progress += 5;
        ...
        if (result.Step1Found && step1StartIndex > 0)
            ...
            result.Message.Add(Ori[step1StartIndex].Day);

        if (result.Step2Found && step2StartIndex > 0)
            // result.KeyData.Add(ori[step2StartIndex]);
            result.Message.Add($"{Ori[step2StartIndex].Day}");
        ...
        if (result.Step3Found && step3StartIndex > 0)
            ...
            result.Message.Add($"{Ori[step3StartIndex].Day}");
        ...
        if (result.Step4Found && step4StartIndex > 0)
            ...
            result.Message.Add($"{Ori[step4StartIndex].Day}");
