...
using SAaP.Models;
using SAaP.Services;
using IChartService = SAaP.Contracts.Services.IChartService;
...
public class AnalyzeDetailViewModel : ObservableRecipient
{
	...
	public AnalyzeDetailViewModel(IStockAnalyzeService stockAnalyzeService, IChartService chartService, IDbTransferService dbTransferService,
	                              IFetchStockDataService fetchStockDataService, IWindowManageService windowManageService)
	{
	...
	public string ComparerCheck
	{
		...
		set
		{
			if (value != null) SetProperty(ref _comparerCheck, value);
		}
	}
	...
	public string ComparerModeCheck
	{
		...
		set
		{
			if (value != null) SetProperty(ref _comparerModeCheck, value);
		}
	}
	...
	public async Task StartAnalyze(object canvas)
	{
		...
		// bring window back
		var xamlRoot = WindowManageService.GetWindowForElement(realCanvas.XamlRoot, typeof(AnalyzeDetailViewModel).FullName!);
	...
	private static void SetHistory(IList<InDayDetail> details, IReadOnlyList<OriginalData> originalDatas)
	{
		...
		while (j < tmpDetail.Count)
		{
			...
			for (var k = 0; k < range; k++)
			{
				...
				while (newDate.DayOfWeek is DayOfWeek.Saturday or DayOfWeek.Sunday) newDate = newDate.AddDays(1);
	...
	private async Task StartDraw(Canvas realCanvas, IReadOnlyList<string> codeList, IReadOnlyList<List<OriginalData>> originalDatasList)
	{
		...
		await Task.Run(async () => { await _dbTransferService.TransferCsvDataToDbAsync(codeList); });
	...
	public async Task Initialize()
	{
		...
		SelectedCompareRelationIndex = 13; // 最近40天
		AnalyzedResults.Clear();

		foreach (var duration in DefaultDuration)
		{
			var data = await _stockAnalyzeService.AnalyzeAsync(CodeName, duration);
			if (data != null) AnalyzedResults.Add(data);
		}
